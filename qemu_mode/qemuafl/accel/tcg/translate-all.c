...
extern double bb_dists[MAP_SIZE];
...
// 원본
  ...
  afl_prev_loc = cur_loc >> 1;
    ...
    //for CVE2016-9827 listswf 
    target_ulong target_func = (0x4000236d >> 4) ^ (0x4000236d << 8);
    //for CVE2018-8807 swftophp
    //target_ulong target_func = (0x4000feb2 >> 4) ^ (0x4000feb2 << 8);
    //for CVE2017-7578 swftophp 
    //target_ulong target_func = (0x400115c7 >> 4) ^ (0x400115c7 << 8);
    //for mjs 73
    //target_ulong target_func = (0x4001b103 >> 4) ^ (0x4001b103 << 8);
    target_func &= MAP_SIZE - 1;

    if(cur_loc==target_func){
        target_ulong a = 0;
        memcpy(&a, afl_area_ptr + MAP_SIZE+24, 4);
        printf("\n\nget!\n\n");
        a += 1;
        memcpy(afl_area_ptr + MAP_SIZE+24, &a, 4);
    }

//   
    //printf("cur_loc in maybe_log : %ld\n",cur_loc);
  // 이거 실행하면 afl-qemu-trace instrumentation doesn't seem to work! 이게 뜨네
//   memset((void*)(afl_area_ptr),1, MAP_SIZE+16);
//   if(bb_dists[cur_loc]>=0){
    double aggr_dist = 0.0, aggr_cnt = 0.0;
    target_ulong a = 0;
    memcpy(&aggr_dist, afl_area_ptr + MAP_SIZE, 8);
    memcpy(&aggr_cnt, afl_area_ptr + MAP_SIZE + 8, 8);
    printf("aggr_dist in maybe_log1 : %f\n",aggr_dist);
    if (aggr_dist ==0.0) aggr_dist=179769313486231570000.0;
    printf("aggr_dist in maybe_log2: %f\n",aggr_dist);
    printf("bb_distance in maybe_log: %f\n",bb_dists[cur_loc]);

    if(bb_dists[cur_loc]!=0.0&&aggr_dist > bb_dists[cur_loc]){
        aggr_dist = bb_dists[cur_loc];
        printf("here\n");
    }
    printf("aggr_dist in maybe_log3 : %f\n",aggr_dist);
    //aggr_dist=1.0;
    aggr_cnt += 1.0;
    ...
    //memcpy(afl_area_ptr + MAP_SIZE+16, &a, 4);
//   }
}
...
static void afl_gen_trace(target_ulong cur_loc) {
  ...
  /* Optimize for cur_loc > afl_end_code, which is the most likely case on
     Linux systems. */
/*if(cur_loc>0x400116a0) {
    //cxxfilt
    target_ulong cur_loc_add = cur_loc+0x80498c0-0x400116a0;
    //opj_decompress(base:0x41e0)
    //target_ulong cur_loc_add = cur_loc+0x2490-0x400116a0;
    printf("hello1 %d , cur_loc_add %p\n",cur_loc,cur_loc_add);
    memcpy(afl_area_ptr + MAP_SIZE+20, &cur_loc_add, 4);
    printf("hello2\n");
}*/
    //memcpy(afl_area_ptr + MAP_SIZE+20, &cur_loc, 4);
...
//memcpy(afl_area_ptr + MAP_SIZE+20, &cur_loc, 4);
  ...
  TCGv cur_loc_v = tcg_const_tl(cur_loc);
    ...
    //printf("cur_loc_v:%ld, cur_loc: %ld\n",cur_loc_v,cur_loc);
