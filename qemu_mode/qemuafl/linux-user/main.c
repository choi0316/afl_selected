...
double bb_dists[MAP_SIZE]; 
static void parse_distance(void){
  // for read distance_file

  char* line = NULL;
  size_t len=0;
  ssize_t read;
  
  FILE* file = fopen("/home/choi/seleted_distance/listswf_9069.txt", "r");
  if (file != NULL) {
    while((read=getline(&line,&len,file)) != -1){
      char* str_addr = strtok(line,",");
      u32 bb_addr = strtoul(str_addr,NULL,16) +0x40000000;
      char* str_dist = strtok(NULL,"");
      double bb_distance =0.0;
      if (str_dist=="inf"){
        bb_distance = 179769313486231570000.0;
      }
      bb_distance = (double) atof(str_dist);
      u32 id = ((bb_addr>>4)^(bb_addr<<8)) & (MAP_SIZE-1);
      
    //   if(bb_distance==0.0){
    //     bb_dists[id]=  DBL_MAX/100000000.0;
    //   }
    //   else if(bb_distance==-1.0){
        bb_dists[id] = bb_distance;
    //   }
    //   else{
    //     bb_dists[id] = 1.0/bb_distance;
    //   }
        //printf("id: %d , bb_dists : %f\n",id,bb_dists[id]);
    }
  }
  else {
      printf("파일을 열 수 없습니다.\n");
  }
  //printf("bb_dists : %f in parse function\n",bb_dists[60957]);
  fclose(file);
...
int main(int argc, char **argv, char **envp)
{
    ...
    parse_distance();
    //printf("fff??\n\n\n");
    // memset((void*)(afl_area_ptr + MAP_SIZE),4, 16);
