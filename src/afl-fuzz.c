...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+a:Ab:B:c:CdDe:E:f:F:g:G:hi:I:l:L:m:M:nNo:Op:P:QRs:S:t:z:"
                       "T:UV:WXx:YZ")) > 0) {
  ...
  afl->argv = use_argv;
  afl->fsrv.trace_bits =
      afl_shm_init(&afl->shm, afl->fsrv.map_size+20, afl->non_instrumented_mode);
  ...
  if (!afl->non_instrumented_mode && !afl->fsrv.qemu_mode &&
    ...
    if (map_size < new_map_size) {
      ...
      afl->fsrv.trace_bits =
          afl_shm_init(&afl->shm, new_map_size+20, afl->non_instrumented_mode);
