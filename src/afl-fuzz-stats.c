...
//static double min_dist= 179769313486231570000.0;
static double max_valid_count = 0.0;
...
#define SP20 SP10 SP10
  ...
  //jmchoi add to print total distance and valid cnt
  
  double* aggr_dist = (double*) (afl->fsrv.trace_bits+afl->fsrv.map_size);
  
  double* valid_count = (double*) (afl->fsrv.trace_bits+afl->fsrv.map_size+8);

  uint32_t* target_function_cnt = (uint32_t*) (afl->fsrv.trace_bits+afl->fsrv.map_size+24);
  
  //if (*aggr_dist<min_dist&&*aggr_dist!=0.0){
  //  min_dist = *aggr_dist;
  //}
  if (max_valid_count<*valid_count){
    max_valid_count = *valid_count;
  }

  //min
  sprintf(tmp, "%f ",*aggr_dist);
  SAYF(bV bSTOP " total_distance : " cRST "%-19s" bSTG bV, tmp);
  //max
  sprintf(tmp, "%f",max_valid_count);
  SAYF(bSTOP " max_valid_cnt : " cRST "%-21s" bSTG bV "\n", tmp);

  sprintf(tmp, "%d",*target_function_cnt);
  SAYF(bSTOP " tf_cnt : " cRST "%-21s" bSTG bV "\n", tmp);
