...
u32 count_bytes(afl_state_t *afl, u8 *mem) {
  ...
  // printf("Total distance : %f, valid_count : %f\n",*aggr_dist, *valid_count);
...
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  //jmchoi
  double* aggr_dist = (double*) (afl->fsrv.trace_bits+afl->fsrv.map_size);
  *(afl->fsrv.trace_bits+afl->fsrv.map_size) = 0.0;
  double* valid_count = (double*) (afl->fsrv.trace_bits+afl->fsrv.map_size+8);
  *(afl->fsrv.trace_bits+afl->fsrv.map_size+8) = 0.0;
  //uint32_t* cur_loc1 = (uint32_t*) (afl->fsrv.trace_bits+afl->fsrv.map_size+24);
  //uint32_t* cur_loc2 = (uint32_t*) (afl->fsrv.trace_bits+afl->fsrv.map_size+20);

  if (*valid_count>0) cur_distance = (*aggr_dist);
  else cur_distance = -1.0;

  //printf("aggr_dist:%f valid_count:%f\n",*aggr_dist,*valid_count);
  //printf("aggr_dist:%f cur_loc2:%x\n",*aggr_dist,*cur_loc2);
  //printf("cur_loc1:%d \n",*cur_loc1);
